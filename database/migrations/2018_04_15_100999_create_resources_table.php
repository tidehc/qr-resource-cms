...
class CreateResourcesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('resources', function (Blueprint $table) {
            ...
            $table->string('product_name', 50)->default('')->comment('物品名称');
            $table->string('menufactoring_number')->default('')->comment('出厂编号')->index();
            $table->string('number_auth')->default('')->comment('编号授权方');
            $table->string('recycle_number')->default('')->comment('回收编号')->index();
            $table->unsignedTinyInteger('toxic')->default(0)->comment('毒害性。默认0，表示无毒害');
            $table->string('poison_category', 50)->default('')->comment('毒害类别');
